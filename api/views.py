...
from .models import GoogleDriveFolder, IngestionRun
...
def list_all_folders(request):
    """
    List all Google Drive folders for the current user.
    
    Shows both folders with data and folders without data.
    """
    try:
        ...
        all_folders = GoogleDriveFolder.objects.filter(
            user=request.user,
            is_active=True
        ).order_by('-created_at')
        ...
        for folder in all_folders:
            ...
            if booking_count > 0:
                ...
                status_text = "Data Available"
            else:
                last_import_qld = "No imports yet"
                status_text = "Ready for Import"
        ...
        return Response({"folders": folder_data_list}, status=status.HTTP_200_OK)
...
def list_inactive_folders_with_data(request):
    """
    List all folders that have imported booking data.
    
    This allows users to access data from all their previously imported folders.
    """
    try:
        # Get Queensland timezone
        qld_tz = pytz.timezone('Australia/Brisbane')
        
        # Get all folders that have booking data
        from .models import Booking
        ...
        return Response({"inactive_folders": folder_data_list}, status=status.HTTP_200_OK)
...
@extend_schema(
    ...
    description="Manually import booking data from Google Sheets in specified folders",
    request={
        "application/json": {
            "type": "object",
            "properties": {
                "folder_id": {
                    "type": "string",
                    "description": "Specific folder ID to process (optional - if not provided, processes all available folders)",
...
def import_booking_data(request):
    """
    Import booking data from Google Sheets in specified folders.
    ...
    1. Discover new Google Sheets files in specified folders
